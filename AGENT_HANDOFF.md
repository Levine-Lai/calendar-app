...
- 2.3.3 的更新下载入口会解析并锁定系统默认浏览器包名后打开 GitHub Release；日历今天格子和日期数字框均改用更深、更明确的两级玫红色。组件1即时刷新改为加急任务、比分先于队徽下载重绘、赛事 ID 变化时按对阵兜底匹配，并执行第 3 版默认日期迁移回北京时间今日。远程更新清单继续停留在已发布的 2.3.2。
