...
1. 把新闻入口从左侧栏移到主界面日历下方，默认显示最近三篇文章。
2. “查看更多”进入新闻界面，列表只保留英文标题和内容概述。
3. 点击文章进入独立全文页，默认英文，右上角切换中英文。
4. 尝试抓取 MLB 新闻图片，并在新闻总览和全文中显示。
5. 去掉通知测试，不影响真实新闻推送。

#### 实现结构

1. `index.html` 删除侧栏新闻面板，在 `calendarView` 后新增主页新闻区；推送开关和状态随新闻入口一同移到主页。
2. 新闻列表固定为英文摘要卡片，不再在列表中直接展开长正文；主页复用同一张卡片组件并限制为最近三篇。
3. 新增全屏文章阅读页，进入时重置为英文，右上角的 `English / 中文` 页签只切换当前文章。
4. 通知点击仍按 MLB URL 定位文章，但现在会直接打开独立全文页。
5. 删除“测试通知”按钮、前端事件和 Android 原生测试通知接口；真实 FCM 与手机后台补偿推送保持不变。
6. 后台从 MLB AMP 页同时抓取正文与主图；普通页 `og:image` 和 AMP 页 `amp-img` 两种结构均支持。
7. 图片只允许 `mlbstatic.com` 与 `mlb.com` 的 HTTPS 地址；旧文章已有正文但缺图时会单独补抓，不重复覆盖已保存正文。
8. MLB 大图地址统一请求 640px 版本，降低主页首次展示和手机流量开销。
9. 静态新闻与 Android 直接 RSS 合并时保留静态数据中的封面、中英文摘要和全文。

#### 实际核验

1. MLB 普通文章页提供 `og:image`，后台使用的 AMP 页实际通过 `amp-img` 提供主图。
...
### 修改批次：2.2.8 手机新闻排版、边缘返回与静默同步

#### 用户反馈

1. 手机首页三列新闻卡片过窄，图片和正文被挤压。
2. 希望卡片占满横向空间，图片在上，标题与内容概述在下。
3. 希望新闻页面适配手机屏幕边缘滑动返回。
4. 不再显示“已自动同步”等无必要成功描述，以后界面只保留有操作价值的信息。

#### 修复结构

1. 定位到通用新闻样式位于移动端媒体查询之后，导致手机一列规则被后续三列规则覆盖。
2. 在样式表末尾增加最终移动端约束：主页固定单列，每篇占满宽度，封面为 `16:9`，文字区域置于图片下方。
3. 新闻全文页和列表支持左右屏幕边缘向内滑动返回；手势仅在边缘起始且横向距离达到阈值时触发，避免干扰正文上下滚动。
4. 侧栏手势在弹窗打开时停用，防止返回新闻时误打开左侧栏。
5. Android `MainActivity` 接管系统返回事件，优先调用页面返回处理；全文、新闻列表、日期弹窗、删除弹窗和侧栏均关闭后才执行系统默认返回。
6. 自动新闻刷新成功后清空状态区，不再显示“已自动同步”“已同步”或缓存条数；同步失败和推送权限异常仍正常提示。
7. 源码版本提升为 `2.2.8 / versionCode 30`，本轮按约定不自动打包。

#### 验证

- Web 自动化测试 27 项、新闻后台测试 23 项、稳定性检查 24 项全部通过。
- Android JVM 测试与 Lint 成功；系统返回实现使用 AndroidX `OnBackPressedDispatcher`，兼容手势导航。
- Web 资源已同步进入 Android 工程，但未生成 `2.2.8` APK。
...
- 在每篇展开的新闻文章最下面增加可点击的收起功能。

#### 实现方案

1. 正文成功渲染后，在段落列表末尾生成“收起文章 / Collapse article”按钮，标签跟随当前中文或 English 页面。
2. 点击底部按钮后同步把文章顶部展开按钮的 `aria-expanded` 恢复为 `false`，隐藏正文并把焦点返回文章标题按钮。
3. 视图平滑回到当前文章顶部；系统启用“减少动态效果”时改为无动画定位。
4. 桌面端按钮靠右显示，移动端占满正文宽度并保持至少 42px 点击高度。
5. 新增稳定性检查“新闻正文底部收起”，防止后续重构遗漏事件、函数或样式。

#### 涉及文件与验证

- `public/app.js`
- `public/styles.css`
- `scripts/verify-stability.js`
- `CHANGELOG.md`
- `context.md`
- Web 自动化测试 26 项全部通过；稳定性检查 23 项全部通过；`public/app.js` 语法检查和 Web 构建成功。
- 当前手机最新 APK 仍为 `2.2.6 / versionCode 28`；当前源码为 `2.2.7 / versionCode 29`，本轮不自动打包。
...
### 开发批次：2.3.4 合并远端 2.3.3、组件1直刷与默认浏览器

- 合并远端已发布 2.3.3 及后续自动新闻更新，保留远端的周期任务续跑、赛中约 2 分钟跟进、MLB RSS 最新新闻选择和新闻直达，同时保留本机的加急任务、比分先重绘、赛事 ID 变化后按对阵匹配和日历今日强调。
