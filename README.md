...
当前源码为 `2.3.1 / versionCode 43`，尚未打包；本机 APK 和 GitHub 最新发布版为 `2.3.0 / versionCode 42`。
...
- 阿森纳新闻由 GitHub Actions 定时读取 Arsenal.com 官方文章 Sitemap 与 The Guardian 阿森纳 RSS，只保存标题、作者、图片、短摘要和原文入口；单源异常时沿用旧缓存。
...
1. “观赛日记 组件1”：约 4×4，默认显示明天的关注比赛，支持前一天、后一天、手动刷新和列表滚动。每个组件实例独立保存日期，队徽和实时比分按日期缓存。
2. “观赛日记 组件2”：4×3，只显示最新一篇蓝鸟新闻的大图和标题。图片占据上方主要区域、保持完整画面并使用顶部圆角，标题位于下方；点击打开对应文章。组件并行读取 Raw/CDN，每 15 分钟独立检查更新，失败时保留已有缓存。
