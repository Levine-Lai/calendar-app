...
当前源码为 `2.3.2 / versionCode 44`，尚未打包；本机 APK 和 GitHub 最新发布版仍为 `2.3.1 / versionCode 43`。
...
2. “观赛日记 组件2”：4×3，只显示最新一篇蓝鸟新闻的大图和标题。图片与标题区域约为 5:2，图片占据上方主要区域、保持完整画面并使用顶部圆角；点击打开对应文章。组件并行读取 Raw/CDN，每 15 分钟独立检查更新，失败时保留已有缓存。
